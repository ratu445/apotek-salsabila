@extends('layouts.app', ['title' => 'Edit Resep'])
        ...
        <form action="{{ route('resep.update', $resep->id_resep) }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('nama_pasien') is-invalid @enderror" 
                           id="nama_pasien" name="nama_pasien" value="{{ old('nama_pasien', $resep->nama_pasien) }}" required>
                    @error('nama_pasien')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control @error('tgl_resep') is-invalid @enderror" 
                           id="tgl_resep" name="tgl_resep" value="{{ old('tgl_resep', $resep->tgl_resep->format('Y-m-d')) }}" required>
                    @error('tgl_resep')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('diagnosa') is-invalid @enderror" 
                          id="diagnosa" name="diagnosa" rows="2">{{ old('diagnosa', $resep->diagnosa) }}</textarea>
                @error('diagnosa')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('catatan') is-invalid @enderror" 
                          id="catatan" name="catatan" rows="2">{{ old('catatan', $resep->catatan) }}</textarea>
                @error('catatan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                <div class="obat-item mb-3 border p-3 rounded" data-index="{{ $index }}">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select select-obat" name="obat[{{ $index }}][id_barang]" required>
                                <option value="">Pilih Obat</option>
                                @foreach($barangs as $barang)
                                <option value="{{ $barang->id_barang }}" 
                                    data-harga="{{ $barang->harga }}"
                                    {{ $item->id_barang == $barang->id_barang ? 'selected' : '' }}>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="text" class="form-control" 
                                   ...
                                   value="{{ $item->aturan_pakai }}" required>
                        </div>
...
<script>
    $(document).ready(function() {
        let obatCounter = {{ count($resep->resepItems) }};
